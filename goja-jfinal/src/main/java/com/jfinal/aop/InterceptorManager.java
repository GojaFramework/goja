...
public class InterceptorManager {
    ...
    // 单例拦截器
    private final ConcurrentMap<Class<? extends Interceptor>, Interceptor> singletonMap =
    ...
    public Interceptor[] buildControllerActionInterceptor(Interceptor[] classInters,
    ...
    public Interceptor[] buildServiceMethodInterceptor(Interceptor[] injectInters,
                                                       Class<?> serviceClass, Method method) {
        return doBuild(globalServiceInters, injectInters, createServiceInterceptor(serviceClass),
                serviceClass, method);
    }

    private Interceptor[] doBuild(Interceptor[] globalInters, Interceptor[] injectInters,
                                  Interceptor[] classInters, Class<?> targetClass, Method method) {
        ...
        ArrayList<Interceptor> result = Lists.newArrayListWithCapacity(
                globalInters.length + injectInters.length + classInters.length + methodInters.length);
    ...
    private void removeInterceptor(ArrayList<Interceptor> target,
                                   Class<? extends Interceptor>[] clearInters) {
        ...
        for (Interceptor inter : inters) {
            ...
            if (singletonMap.containsKey(inter.getClass())) {
                throw new IllegalArgumentException(
                        "interceptor already exists, interceptor must be singlton, do not create more then one instance of the same Interceptor Class.");
            }
        }
