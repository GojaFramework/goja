...
import goja.core.tuples.valueintf.IValueLabel;
import goja.core.tuples.valueintf.IValueValue;

import java.util.Collection;
import java.util.Iterator;

/**
 * <p> A tuple of two elements, with positions 0 and 1 renamed as "label" and "value", respectively.
 * </p>
 *
...
public final class LabelValue<A, B>
    ...
    public static <X> LabelValue<X, X> fromArray(final X[] array) {
        ...
        if (array.length != 2) {
            throw new IllegalArgumentException(
                    "Array must have exactly 2 elements in order to create a LabelValue. Size is "
                            + array.length);
        }
    ...
    private static <X> LabelValue<X, X> fromIterable(final Iterable<X> iterable, int index,
        ...
        if (tooFewElements && exactSize) {
            throw new IllegalArgumentException(
                    "Not enough elements for creating a LabelValue (2 needed)");
        }

        if (iter.hasNext() && exactSize) {
            throw new IllegalArgumentException(
                    "Iterable must have exactly 2 available elements in order to create a LabelValue.");
        }
