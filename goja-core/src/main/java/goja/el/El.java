...
import goja.el.arithmetic.RPN;
import goja.el.arithmetic.ShuntingYard;
...
public class El {
     ...
     * 对参数代表的表达式进行运算
     */
    public static <T> T  eval(String val) {
        //逆波兰表示法（Reverse Polish notation，RPN，或逆波兰记法）
        return eval(null, val);
    }

    public static <T> T eval(Context context, String val) {
        ShuntingYard sy = new ShuntingYard();
        RPN rc = new RPN();
        Queue<Object> rpn = sy.parseToRPN(val);
        return (T) rc.calculate(context, rpn);
    }
