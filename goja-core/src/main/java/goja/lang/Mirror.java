...
public class Mirror<T> {
    ...
    public Method getGetter(String fieldName) throws NoSuchMethodException {
        String fn = Strs.upperFirst(fieldName);
        String _get = "get" + fn;
        String _is = "is" + fn;
    ...
    public static void evalGetterSetter(Method method,
        ...
        if (name.startsWith("get") && method.getParameterTypes().length == 0) {
            name = Strs.lowerFirst(name.substring(3));
            getter = method;
            // 寻找 setter
            try {
                setter = method.getDeclaringClass().getMethod("set" + Strs.upperFirst(name),
        ...
        // 布尔的 getter
            ...
            name = Strs.lowerFirst(name.substring(2));
            getter = method;
            // 寻找 setter
            try {
                setter = method.getDeclaringClass().getMethod("set" + Strs.upperFirst(name),
        ...
        // 是 setter
            ...
            // 寻找 getter
            try {
                getter = method.getDeclaringClass().getMethod("get" + Strs.upperFirst(name));
            }
            catch (Exception e) {}
    ...
    public Method getSetter(String fieldName, Class<?> paramType) throws NoSuchMethodException {
        try {
            String setterName = "set" + Strs.upperFirst(fieldName);
            try {
    ...
    public Method[] findSetters(String fieldName) {
        String mName = "set" + Strs.upperFirst(fieldName);
        List<Method> ms = Lists.newArrayList();
    ...
    public static Class<?>[] getGenericTypes(Field f) {
        ...
        if (m.find()) {
            String s = m.group(2);
            String[] ss = Strs.splitIgnoreBlank(s);
            if (ss.length > 0) {
