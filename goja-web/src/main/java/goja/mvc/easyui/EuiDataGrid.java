...
import goja.mvc.easyui.rsp.DataGridRsp;
...
public class EuiDataGrid {
    ...
    public static final DataGridRsp EMPTY_DATAGRID  =
            new DataGridRsp.Builder().rows(Collections.<Record>emptyList()).total(0).build();
    private static final  String      EASYUI_DATAGRID = ".easyui.datagrid";
    ...
    public static Optional<DataGridReq> req(HttpServletRequest request) {
        if (request != null) {
            ...
            int page =
                    Strings.isNullOrEmpty(r_page) ? 1 : MoreObjects.firstNonNull(Ints.tryParse(r_page), 1);
            page = (page == 0) ? 1 : page;
            int rows =
                    Strings.isNullOrEmpty(r_rows) ? 10 : MoreObjects.firstNonNull(Ints.tryParse(r_rows), 10);
            rows = (rows == 0) ? 10 : rows;
...
//            String p_index = null;
    ...
    public static DataGridRsp rsp(DataGridReq req, String sqlGroupName, List<Object> params) {
        ...
        final Page<Record> paginate =
                Db.paginate(page, pageSize, sqlNode.selectSql, where.toString(), params.toArray());
        DataGridRsp.Builder builder = new DataGridRsp.Builder();
        builder.rows(paginate.getList()).total(paginate.getTotalRow());
        return builder.build();
    }
